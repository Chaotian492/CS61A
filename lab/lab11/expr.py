...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        now_operator = self.operator.eval(env)
        value_list = []
        for element in self.operands:
            now_operands = element.eval(env)
            value_list.append(now_operands)
        return now_operator.apply(value_list)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        # create parent's copy, avoid polluting the parent frame
        # we eval this lambda function in 'lambda frame', that is parent's child
        parent_env_copy = self.parent.copy()
        # update parameters to current env_copy
        parent_env_copy.update(zip(self.parameters, arguments))
        # call body.eval to eval lambda function
