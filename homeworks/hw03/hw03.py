...
def balanced(m):
    ...
    if is_planet(m):
        ...
        total_left = length(left(m)) * total_weight(left_m)
        total_right = length(right(m)) * total_weight(right_m)
        return balanced(left_m) and balanced(right_m) and total_left == total_right
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        label_m = total_weight(m)
        branch = [totals_tree(end(left(m))), totals_tree(end(right(m)))]
        return tree(label_m, branch)
...
def has_path(t, word):
    ...
    #         return True
    #     return False
    if label(t) is not word[0]:
        return False
    else:
        next_word = word[1:]
        if next_word == "":
            return True
        return any([has_path(b, next_word) for b in branches(t)])
