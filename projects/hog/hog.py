...
def swine_align(player_score, opponent_score):
    ...
    while divisor <= min(player_score, opponent_score):
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        say_last, say_high = last_score, running_high
        if who == 0:
            high = score0 - say_last
            say_last = score0
        else:
            high = score1 - say_last
            say_last = score1
        if high > say_high:
            say_high = high
            print(say_high, "point(s)! The most yet for Player", who)
        return announce_highest(who, say_last, say_high)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_func(*args):
        ...
        for _ in range(trials_count):
            ...
            sum_all += result
        # print(sum_all)
        averaged_sum = sum_all / trials_count
        return averaged_sum
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_scoring, flag, scoring = 0, 0, 0
    for i in range(1, 11):
        scoring = averaged_roll_dice(i, dice)
        flag = i if scoring > max_scoring else flag
        max_scoring = scoring if scoring > max_scoring else max_scoring
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    # BEGIN PROBLEM 10
    return (
        0 if free_bacon(opponent_score) >= cutoff else num_rolls
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    return (
        ...
        or free_bacon(opponent_score) >= cutoff
        else num_rolls
    )  # Replace this statement
    # END PROBLEM 11
